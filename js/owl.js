...
resource = {
  '@context': {
    ...
    'DOLCE-Lite.owl': 'http://www.loa-cnr.it/ontologies/DOLCE-Lite.owl#'
  },
  'reverse_c': {},
  'term': {},
  'metadata': {}
...
function process_ontology(store) {
  ...
  // Change the parent of the current top node to owl:Thing
  store.remove(top_node, subClassOf, undefined);
  store.add(top_node, subClassOf, rdflib_url("owl:Thing"));
  ...
  while (stack.length) {
    ...
    var node = {
      ...
      'restrictionTriples': [], // Initialize the restrictionTriples array
    };

    // ... other property extraction calls ...

    // Extract restriction triples
    get_restrictions(resource, node, store, subject);
    //console.log(node.id)
    //console.log(node.restrictionTriples)
...
function get_restrictions(resource, node, store, subject) {
  ...
  restrictions.forEach(function (restriction) {
    if (store.holds(restriction, rdflib_url('rdf:type'), rdflib_url('owl:Restriction'))) {
      var onProperty = store.any(restriction, rdflib_url('owl:onProperty'));
      var someValuesFrom = store.any(restriction, rdflib_url('owl:someValuesFrom'));
      ...
      if (onProperty) {
        ...
        if (allValuesFrom) {
          restrictionTriple.allValuesFrom = namespace(resource, allValuesFrom.uri);
        }

        if (!node.restrictionTriples) {
          node.restrictionTriples = [];
        }

        node.restrictionTriples.push(restrictionTriple);
      }
    }
  });
}
